...
def fetch_data():
    ...
    print(soup)
    contents = ["https://fastpaydayloansfloridainc.com" + a_tag["href"] for a_tag in soup.find("div", {"id": "leftcontent"}).find_all("a") if "Payday" in a_tag.text.strip().split(" ")[0].lower()]
    print(contents)
    for row in contents:
        url = row["href"]
        print(url)
